...
The +signitures+ is a list of a methods documented argument interfaces.
The first entry MUST be thew literal interface as defined in source code.
The remaing entries, if any, are a list of _intendend_ interfaces for a
...
Might document the interface as `x(name, opts={})` in addition to the 
literal `x(*args)`.

Signature entries themselves are a mapping of `image`, `arguments`, `parameters`
and `block` entires:
